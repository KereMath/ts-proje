"""
ensemble-alldata (CONTEXTUAL-OFF) - Konfigurasyon
9 binary classifier: 4 base tip + 5 anomali tipi (contextual_anomaly KALDIRILDI).
"""
from pathlib import Path

BASE_DIR           = Path(__file__).parent
# Veri kaynagi: ana ts-proje runner generated klasoru (Grup 6 atlanir)
...
N                  = 600  # 10x        # pos + neg per model (kucuk veriseti)
...
# -------------------------------------------------------------------
# 38 Kaynak Grubu (Grup 6 = contextual_anomaly KALDIRILDI)
# -------------------------------------------------------------------
SOURCE_GROUPS = [
...
# Yardimci: tum grup id'leri (Grup 6 yok, ama numara atlanmiyor)
ALL_GROUP_IDS = {gid for gid, _, _ in SOURCE_GROUPS}

# -------------------------------------------------------------------
# Her modelin pozitif kaynak grubu ID'leri (Grup 6 cikarildi)
# CONTEXTUAL kaldirildi
# -------------------------------------------------------------------
MODEL_POSITIVE_GROUPS = {
    "stationary":          {1, 5, 7, 8, 9, 10},  # Grup 6 cikti
...
# 9 model (contextual_anomaly cikti)
ALL_MODEL_NAMES = [
...
ANOMALY_MODELS = ["collective_anomaly", "mean_shift",
                  "point_anomaly", "trend_shift", "variance_shift"]
