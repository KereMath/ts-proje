...
def prepare_meta_data(force: bool = False):
    ...
    n = len(series_list)
    raw_old = np.zeros((n, 8))  # contextual cikti
    raw_new = np.zeros((n, 9))  # contextual cikti
    ...
    print(f"  Meta-features boyut: {meta_features.shape[1]} "
          f"(8 old + 9 new + {derived.shape[1]} derived + 777 tsfresh)")

    # --- 5. Ground truth etiketler (contextual yok) ---
    BASE_MAP = {"stationary": 0, "deterministic_trend": 1,
                "stochastic_trend": 2, "volatility": 3}
    ANOM_LIST = ["collective_anomaly", "mean_shift",
                 "point_anomaly", "trend_shift", "variance_shift"]

    y_base = np.zeros(n, dtype=int)
    y_anom = np.zeros((n, 5), dtype=int)  # 5 anomali (contextual cikti)
...
def _compute_derived_features(raw_old: np.ndarray, raw_new: np.ndarray) -> np.ndarray:
    """
    17 ham olasiliktan turetilmis meta-feature'lar (8 old + 9 new, contextual cikti).
    Her satir icin ~14 ek feature.
    """
    n = raw_old.shape[0]
    feats = []

    # OLD_CLASSES (contextual-off, 8 class):
    #   0=collective, 1=det_trend, 2=mean_shift, 3=point, 4=stoch_trend,
    #   5=trend_shift, 6=variance_shift, 7=volatility
    old_base_idx = [1, 4, 7]            # det_trend, stoch_trend, volatility
    old_anom_idx = [0, 2, 3, 5, 6]      # collective, mean, point, trend, variance (5)

    # NEW_ALL_MODELS (contextual-off, 9 model):
    #   0-3 = base (stationary, det, stoch, vol)
    #   4-8 = anomaly (collective, mean, point, trend, variance) (5)
    new_base_idx = [0, 1, 2, 3]
    new_anom_idx = [4, 5, 6, 7, 8]
