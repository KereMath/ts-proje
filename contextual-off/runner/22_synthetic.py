"""
50 sentetik stochastic-trend serisini contextual-off ens-final'a sok.
"""
import json
import sys
import warnings
...
SYN_DIR = Path(r"c:\Users\Pc\Desktop\ceylanhoca\runner\data\synthetic")
...
def main():
    ...
    print(f"[1] {len(items)} sentetik seri")
    X = extract_batch(series_list)
    X_clean = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)

    print("[2] Modeller yukleniyor...")
    ...
    for i, it in enumerate(items):
        ...
        rec = {
            **it, "expected_class": "stochastic_trend",
    ...
    with open(OUT / "synthetic.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    df = pd.DataFrame([{
        "kind": r["kind"], "n": r["n"], "correct": r["base_correct"], "pred": r["pred_base"],
    } for r in results])
    print("\nSentetik accuracy:")
    print(df.groupby(["kind", "n"])["correct"].mean().to_string())
    print(f"\nTOPLAM: {df['correct'].sum()}/{len(df)} ({df['correct'].mean():.0%})")
