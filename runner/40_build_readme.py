...
def build_readme():
    ...
    out.append("")
    out.append("### KRITIK BULGU: Stationarity gate yanlis tetikleniyor")
    out.append("")
    out.append("**14 sentetik stochastic serisi icin** stat_gate (P(stationary) >= 0.92) tetiklenip base'i ZORLA `stationary` yapiyor — halbuki base meta-learner ayni serilerde `stochastic_trend` diyor (b_str=0.99).")
    out.append("")
    out.append("Ornek: `ari_L45_00.csv`")
    out.append("```")
    out.append("base meta-learner:  P(stationary)=0.00, P(stoch_trend)=0.99  -> argmax = stoch")
    out.append("21-feat stat-gate:  P(stationary)=0.963                       -> gate fires (>=0.92)")
    out.append("FINAL              -> stationary  (gate override)")
    out.append("```")
    out.append("")
    out.append("**Gate kapali senaryosu (sadece base meta argmax):**")
    out.append("")
    out.append("| kind/n | mevcut acc | gate OFF acc |")
    out.append("|---|---|---|")
    out.append("| ari 45 | %20 | **%60** |")
    out.append("| ari 100 | **%0** | **%80** |")
    out.append("| arima 45 | %20 | %20 |")
    out.append("| arima 100 | %20 | **%100** |")
    out.append("| ima 45 | %20 | **%80** |")
    out.append("| ima 100 | %60 | **%80** |")
    out.append("| rw 45 | %40 | %40 |")
    out.append("| rw 100 | %100 | %100 |")
    out.append("| rwd 45 | %80 | %80 |")
    out.append("| rwd 100 | %80 | %80 |")
    out.append("| **TOPLAM** | **%44** | **%72** |")
    out.append("")
    out.append("**Yorum:**")
    out.append("- 21-feature stationary detector (`stationary-detection-ml/best_model.pkl`) bizim eğitim datamızdan FARKLI bir veri setiyle (276K orijinal) egitildigi icin OOD davranisi var.")
    out.append("- Kisa ARI/IMA/ARIMA serilerinde drift sinyali zayif kaliyor; statistical features (rolling_std vb.) stationary'e benziyor; gate yanlis tetikleniyor.")
    out.append("- Ironik: ayni gate **realdata icin hicbir zaman acilmadi** (37/37 combo yoluna gitti). DAX log returns (kesin stationary) icin bile P(stat)=0.006.")
    out.append("- **Tedavi:** Gate threshold 0.92 -> 0.97 yukseltmek veya gate'i kaldirmak. Veya base meta P(stationary) ile birlikte AND kontrolu (her ikisi de >0.7 ise stationary).")
    out.append("")
    ...
    if gt is not None:
        ...
        out.append("")
    out.append("**Ana bulgular (FIX SONRASI):**")
    out.append("- **3 FULL match** (NP_xetradax_returns100, US_investment, strikes) — hepsi PDF'te kesin stationary etiketli")
    out.append("- 5 PARTIAL (base dogru ama anomali fazlasi: W3, W5, W6, uspop, GermanGNP)")
    out.append("- Base type **8 / 20** (~%40) dogru — DAX returns, US_investment, strikes basariyla stationary olarak tanindi")
    ...
    out.append("- W1 (stationary+point) hala yanlis: stochastic_trend deniyor — bu base meta'nin gercek hatasi")
    ...
    out.append("")
    out.append("**Fix oncesi vs sonrasi karsilastirma:**")
    out.append("")
    out.append("| | Fix oncesi | Fix sonrasi |")
    out.append("|---|---|---|")
    out.append("| FULL | 0 | **3** |")
    out.append("| PARTIAL | 8 | 5 |")
    out.append("| NONE | 11 | 12 |")
    out.append("| MISSING | 2 | 1 (rec_dataframe indirildi) |")
    out.append("")
    out.append("Uygulanan 3 fix:")
    out.append("1. **CONTEXT_THRESH 0.0 -> 0.55**: stationary base'de anomali threshold'u dusurmek yerine yukseltmek (bug fix)")
    out.append("2. **Stationarity gate + base meta AND**: gate sadece base meta P(stationary) >= 0.40 ise tetikleniyor (sentetik yanlis-pozitifi kes)")
    out.append("3. **Anomali suppress**: base meta P(stationary) >= 0.75 ise stationary base'de anomali listeleme")
    out.append("")
    out.append("---")
    out.append("")
